...
$(document).ready(function(){
    ...
    function check_username_ajax(username){
        $("#usernameCheck").html('');
        $.post('users/parsers/existingUsernameCheck.php', {'username': username}, function(response) {
            if (response == 'error') $('#usernameCheck').html('There was an error while checking the username.');
            
            else if (response == 'taken') {  
                $('#usernameCheck').html('<i class="glyphicon glyphicon-remove" style="color: red; font-size: 12px"></i> This username is taken.');
                $('#next_button').prop('disabled', true); 
            }

            else if (response == 'valid') { $('#usernameCheck').html('');
...
<script type="text/javascript" src="<?php echo $jscdn ?>/vendor/jquery-validation.js"></script>

<script type="text/javascript" src="<?php echo $jscdn ?>/vendor/jquery-validation-additional-methods.js"></script>
<script>
